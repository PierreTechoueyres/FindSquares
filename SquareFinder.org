...
* Analyse
  Pour chaque coupe de point (A, B) dans le plan il est possible de
  déterminer quatre autres point A' (1 et 2) et B' (1 et 2) tels qu'à eux 8 ils forment deux carrés.

  Dans l'exemple précédent si A = (-1, -1) et B = (2, 1) alors il est
  ...
  À partir cela il est possible de rechercher dans la liste des points
  s'il existe des points correspondants à A' et B'.
...
❯ cat ../../exercice.txt| sort -n | uniq -dc
   ...
   le fichier et de ne conserver qu'une seule occurrence.
...
* Améliorations
  J'ai l'intuition qu'il est possible d'arrêter les parcours des
  points A et B en utilisant les propriétés du plan (min et max des
  coordonnées) mais je n'ai pas réussi à trouver comment.
